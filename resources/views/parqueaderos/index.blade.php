...
<main class="max-w-6xl mx-auto p-6"
                ...
                <div class="flex justify-between items-start">
                    ...
                    @if($usuario && $usuario->email === 'admin@gmail.com')
                        <div class="flex flex-col gap-2 ml-4">
                            <button
                                @click="modalOpen = true; selectedParqueadero = {{ $p->toJson() }};"
                                class="bg-yellow-400 hover:bg-yellow-500 text-white px-3 py-1 rounded shadow text-sm text-center">
                                ✏️ Editar
                            </button>

                            <button
                                @click="modalDeleteP = true; selectedParqueadero = {{ $p->toJson() }};"
                        ...
                        @else
                            <div class="flex flex-col gap-2">
                                @foreach($p->reservas as $reserva)
                                    <div class="bg-blue-50 border border-blue-200 rounded-lg px-3 py-2 shadow-sm flex justify-between">
                                        <div>
                                        ...
                                        <button
                                            @click="modalDeleteR = true; selectedReserva = {{ $reserva->toJson() }};"
    ...
    <!-- ✅ Modal Borrar Parqueadero -->
    <div x-show="modalDeleteP" x-transition x-cloak
         class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
        <div class="bg-white rounded-lg shadow-lg p-6 w-96">
            <h3 class="text-lg font-bold text-red-600 mb-4">Eliminar Parqueadero</h3>
            <p class="text-gray-700 mb-4">
                ¿Seguro que deseas eliminar el parqueadero
                <strong x-text="selectedParqueadero.nombre"></strong>?
            </p>

            <form :action="`/parqueadero/eliminar/${selectedParqueadero.id}`" method="POST">
                @csrf @method('DELETE')

                <div class="flex justify-end gap-2">
                    <button type="button" @click="modalDeleteP = false"
                        class="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400">Cancelar</button>
                    <button type="submit"
    ...
    <div x-show="modalDeleteR" x-transition x-cloak
         class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
        <div class="bg-white rounded-lg shadow-lg p-6 w-96">
            <h3 class="text-lg font-bold text-red-600 mb-4">Eliminar Reserva</h3>
            <p class="text-gray-700 mb-4">
                ¿Eliminar la reserva de
                <strong x-text="selectedReserva.nombre_usuario"></strong>?
            </p>

            <form :action="`/reserva/eliminar/${selectedReserva.id}`" method="POST">
                @csrf @method('DELETE')

                <div class="flex justify-end gap-2">
                    <button type="button" @click="modalDeleteR = false"
                        class="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400">Cancelar</button>
                    <button type="submit"
                        class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">Eliminar</button>
                </div>
